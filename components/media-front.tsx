...
export function MediaFront() {
  ...
  return <QiyuAppShell active="media" eyebrow="栖屿影音" title="影音">
    <div className="space-y-7">
      ...
      <section className="flex flex-col justify-between gap-4 border-b border-white/7 pb-6 md:flex-row md:items-end"><div><p className="text-sm text-primary">本地影音资料库</p><h2 className="mt-1.5 text-3xl font-medium tracking-[-0.045em]">随时开始，也随时继续。</h2><p className="mt-2 text-sm text-muted-foreground">从自己的 NAS 播放，媒体整理与转码都在后台完成。</p></div><div className="flex flex-wrap gap-2"><Button onClick={() => void load(activeLibrary)} variant="outline" className="rounded-lg border-white/10 bg-white/[0.02]"><RefreshCw className="size-4" />刷新</Button><a href="/collect" className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-primary/25 bg-primary/10 px-3 text-sm font-medium text-primary hover:bg-primary/15"><Download className="size-4" />采集影音</a><a href="/files?path=media" className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-primary px-3 text-sm font-medium text-primary-foreground hover:bg-primary/80"><Plus className="size-4" />导入文件</a></div></section>
