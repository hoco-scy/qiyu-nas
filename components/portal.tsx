...
import { Activity, ArrowUpRight, Cpu, Download, FolderOpen, HardDrive, MemoryStick, Play, Plus, RefreshCw, Timer, Upload } from 'lucide-react';
  ...
  return <QiyuAppShell active="home" eyebrow="栖屿空间" title="总览">
    <div className="space-y-6">
      <section className="flex flex-col gap-4 border-b border-white/7 pb-6 md:flex-row md:items-end md:justify-between">
        <div><p className="text-sm text-primary">{date}</p><h2 className="mt-1.5 text-3xl font-medium tracking-[-0.045em] sm:text-[2.1rem]">你的空间，一目了然。</h2><p className="mt-2 text-sm text-muted-foreground">文件、影音、采集与网页，都在同一个入口里。</p></div>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Metric icon={Cpu} label="CPU" value={loading ? '—' : `${status.cpu.percent}%`} note={status.cpu.cores ? `${status.cpu.cores} 核可用` : '正在读取运行状态'} />
        ...
        <Metric icon={Timer} label="运行时间" value={status.uptime ? formatUptime(status.uptime) : '—'} note={`${status.hostname} 正在运行`} />
      </section>

      <section className="grid gap-4 xl:grid-cols-[1.35fr_0.65fr]">
        <div className="rounded-2xl border border-white/8 bg-white/[0.025] p-5 sm:p-6"><div className="flex items-start justify-between"><div><p className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">快速开始</p><h3 className="mt-2 text-xl font-medium tracking-[-0.025em]">从哪里开始？</h3></div><Activity className="size-5 text-primary" /></div><div className="mt-5 grid gap-2 sm:grid-cols-2 xl:grid-cols-4"><QuickLink href="/files" icon={Plus} title="整理文件" note="上传、预览和移动" /><QuickLink href="/media" icon={Play} title="打开影音" note="浏览并直接播放" /><QuickLink href="/collect" icon={Download} title="采集影音" note="从公开链接保存" /><QuickLink href="/websites" icon={ArrowUpRight} title="发布网页" note="管理静态站点" /></div></div>
